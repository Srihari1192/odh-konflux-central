...
def ok_cases() -> list[OkCase]:
    return [
        OkCase(
            [],
            check=lambda a: _checks(a.product == "existing", a.list_pipelines == 0, not a.watch_mode),
            id="defaults",
        ),
        OkCase(
            ["--product", "odh"],
        ...
        OkCase(
            ["--tests", "bvt"],
            check=lambda a: _checks(a.product == "existing", a.tests == "bvt", not a.watch_mode),
            id="tests-bvt",
        ),
        OkCase(
            ["--tests", "smoke"],
        ...
        OkCase(["--tests", "smoke", "--test-timeout", "10m"], check=lambda a: _checks(a.test_timeout == "10m"), id="test-timeout-10m"),
        ...
        OkCase(
            ["--konflux-namespace", "ns1", "--konflux-app", "app1", "--channel", "ch1"],
        ...
        OkCase(["--tests", "bvt", "--channel", "stable"], check=lambda a: _checks(a.tests == "bvt"), id="bvt-channel"),
...
def err_cases() -> list[ErrCase]:
    return [
        ...
        ErrCase(["-l", "--channel", "x"], "Trigger/install options cannot be used", id="list-channel"),
        ErrCase(
        ...
        ErrCase(["--cleanup", "--tests", "smoke"], "requires --external-kubeconfig", id="cleanup-without-kubeconfig"),
        ErrCase(["-l", "--cleanup"], "Trigger/install options cannot be used", id="list-cleanup"),
    ]
