#!/usr/bin/env python3
"""Tests for CodeFlare SDK EaaS kubeconfig auth wrapper."""
...
from components.codeflare_sdk.eaas import (  # noqa: E402
    codeflare_eaas_kubeconfig_run_prefix,
    codeflare_eaas_run_tests_auth_patch_shell,
    prepend_codeflare_eaas_kubeconfig_auth,
)

class CodeflareSdkEaasTest(unittest.TestCase):
    def test_prefix_unsets_legacy_vault_vars(self) -> None:
        with mock.patch.dict("os.environ", {"CLUSTER_SOURCE": "EAAS", "OPENSHIFT_TOKEN": "tok"}, clear=False):
            with mock.patch("components.codeflare_sdk.eaas._cluster_is_byoidc", return_value=False):
                with mock.patch("components.codeflare_sdk.eaas.cluster_has_htpasswd_identity", return_value=False):
    ...
    def test_run_tests_patch_sets_byoidc_when_cluster_auth_openshift(self) -> None:
        shell = codeflare_eaas_run_tests_auth_patch_shell()
    ...
    def test_prepend_wraps_run_command(self) -> None:
        with mock.patch(
            "components.codeflare_sdk.eaas.codeflare_eaas_kubeconfig_run_prefix",
            return_value="export CLUSTER_AUTH=openshift; ",
        ):
            out = prepend_codeflare_eaas_kubeconfig_auth("bash run-tests.sh -m smoke")
